...
void lnh_sw_reset() {
    ...
    for (unsigned long long int i=1; i<lnh_s_count; i++) lnh_del_str_sync(i);
    //Reset TSC counter
    LNH_CNTL = (1ull<<RESET_TSC);
}
...
void lnh_sync() {
	...
	while ((LNH_STATE & ( 1ull<<SPU_ALL_DONE  )) == 0ull );
    //do {lnh_rd_reg64_byval(LNH_STATE); wait;} while ((lnh_core.buf64 & ( 1ull<<SPU_ALL_DONE  )) == 0ull);
}
...
void lnh_syncm(int mbr) {
    ...
    while ((LNH_STATE & ( 1ull<<MBOX_VFLAG[mbr]  )) == 0ull );
    //do {lnh_rd_reg64_byval(LNH_STATE); wait;} while ((lnh_core.buf64 & ( 1ull<<MBOX_VFLAG[mbr] )) == 0ull);
}
...
bool lnh_del_sync(uint64_t str, uint64_t key) {
    ...
    //запись исходных данных
	    KEY2LNH = key;
        CMD2LNH = (DEL<<lnh_cmd)|str;

    //ожидание готовности очереди команд
		lnh_sync();

    //чтение результата
	    lnh_core.result.key = LNH2KEY;
	    lnh_core.result.value = LNH2VAL;
	    lnh_core.result.status = LNH_STATE;
...
bool lnh_get_q() {
	...
	//ожидание готовности очереди
 	    ...
 	    while ((LNH_STATE & (1<<SPU_ALL_DONE))==0);
...
bool lnh_ins_syncm(int st_mreg, uint64_t str, uint64_t key, uint64_t value)
{
    ...
    //запись исходных данных
	    KEY2LNH = key;
        VAL2LNH = value;
    	CMD2LNH =  (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(INS<<lnh_cmd)|str;
...
bool lnh_del_syncm(int st_mreg, uint64_t str, uint64_t key) {
    ...
    //запись исходных данных
	    KEY2LNH = key;
        CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(DEL<<lnh_cmd)|str;
...
uint32_t lnh_get_numm(uint64_t str) {
    //проверка готовности устройства
        ...
        CMD2LNH = (mrf<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(NOP<<lnh_cmd)|str;
...
bool lnh_del_str_syncm(int st_mreg, uint64_t str){
    ...
    //запись исходных данных
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(DELSTR<<lnh_cmd)|str;
...
bool lnh_sq_syncm(int st_mreg, uint64_t str){
    ...
    //запись исходных данных
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(SQUIZ<<lnh_cmd)|str;
...
bool lnh_or_syncm(int st_mreg, uint64_t A, uint64_t B, uint64_t R){
    ...
    //запись исходных данных
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(POR<<lnh_cmd)|str;
...
bool lnh_and_syncm(int st_mreg, uint64_t A, uint64_t B, uint64_t R){
    ...
    //запись исходных данных
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(PAND<<lnh_cmd)|str;
...
bool lnh_not_syncm(int st_mreg, uint64_t A, uint64_t B, uint64_t R){
    ...
    //запись исходных данных
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(PNOT<<lnh_cmd)|str;
...
bool lnh_lseq_syncm(int st_mreg, uint64_t key, uint64_t A, uint64_t R){
    ...
    //запись исходных данных
        KEY2LNH = key;
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(LSEQ<<lnh_cmd)|str;
...
bool lnh_ls_syncm(int st_mreg, uint64_t key, uint64_t A, uint64_t R){
    ...
    //запись исходных данных
        KEY2LNH = key;
        CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(LS<<lnh_cmd)|str;
...
bool lnh_greq_syncm(int st_mreg, uint64_t key, uint64_t A, uint64_t R){
    ...
    //запись исходных данных
        KEY2LNH = key;
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(GREQ<<lnh_cmd)|str;
...
bool lnh_gr_syncm(int st_mreg, uint64_t key, uint64_t A, uint64_t R){
    ...
    //запись исходных данных
        KEY2LNH = key;
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(GR<<lnh_cmd)|str;
...
bool lnh_grls_syncm(int st_mreg, uint64_t key_ls, uint64_t key_gr, uint64_t A, uint64_t R) {
    ...
    //запись исходных данных
        KEY2LNH =  key_ls;
        VAL2LNH =  key_gr;
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(mrf<<lnh_mbox_val)|(mrf<<lnh_mbox_key)|(GRLS<<lnh_cmd)|str;
...
bool lnh_searchm(int key_mreg, int val_mreg, int st_mreg, uint64_t str, uint64_t key){
    ...
    //запись исходных данных
        KEY2LNH = key;
        CMD2LNH = (st_mreg<<lnh_mbox_st)|(val_mreg<<lnh_mbox_val)|(key_mreg<<lnh_mbox_key)|(SEARCH<<lnh_cmd)|str;
...
bool lnh_nextm(int key_mreg, int val_mreg, int st_mreg, uint64_t str, uint64_t key){
    ...
    //запись исходных данных
        KEY2LNH = key;
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(val_mreg<<lnh_mbox_val)|(key_mreg<<lnh_mbox_key)|(NEXT<<lnh_cmd)|str;
...
bool lnh_prevm(int key_mreg, int val_mreg, int st_mreg, uint64_t str, uint64_t key){
    ...
    //запись исходных данных
        KEY2LNH = key;
        CMD2LNH = (st_mreg<<lnh_mbox_st)|(val_mreg<<lnh_mbox_val)|(key_mreg<<lnh_mbox_key)|(PREV<<lnh_cmd)|str;
...
bool lnh_nsmm(int key_mreg, int val_mreg, int st_mreg, uint64_t str, uint64_t key){
    ...
    //запись исходных данных
        KEY2LNH = key;
        CMD2LNH = (st_mreg<<lnh_mbox_st)|(val_mreg<<lnh_mbox_val)|(key_mreg<<lnh_mbox_key)|(NSM<<lnh_cmd)|str;
...
bool lnh_ngrm(int key_mreg, int val_mreg, int st_mreg, uint64_t str, uint64_t key){
    ...
    //запись исходных данных
        KEY2LNH = key;
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(val_mreg<<lnh_mbox_val)|(key_mreg<<lnh_mbox_key)|(NGR<<lnh_cmd)|str;
...
bool lnh_get_firstm(int key_mreg, int val_mreg, int st_mreg, uint64_t str){
    ...
    //запись исходных данных
	    CMD2LNH = (st_mreg<<lnh_mbox_st)|(val_mreg<<lnh_mbox_val)|(key_mreg<<lnh_mbox_key)|(FST<<lnh_cmd)|str;
...
bool lnh_get_lastm(int key_mreg, int val_mreg, int st_mreg, uint64_t str){
    ...
    //запись исходных данных
    	CMD2LNH = (st_mreg<<lnh_mbox_st)|(val_mreg<<lnh_mbox_val)|(key_mreg<<lnh_mbox_key)|(LST<<lnh_cmd)|str;
...
uint64_t lnh_get_m(int mreg) {
    ...
    while ( (LNH_STATE & (1ull<<MBOX_VFLAG[mreg])) !=0 ) ;
